...
namespace OldTigerWeb.DataAccess
{
    public class DATroubleList
    {
        ...
        public DataTable SelectTroubleList(String Mode, String Type, String Moji,DataTable Table, String paraCondition,
            String paraCategoryCondition) // 20170719 Add
        {
            ...
            if (Type == Def.DefTYPE_WORD || Type == Def.DefTYPE_TOP10)
            {
                // 文字列検索
                // TOP10検索
                strWork = "";
                ...
                {   
                    ...
                    if (i != 0)
                    {
                        if (paraCondition == Def.DefTYPE_AND)
                        {
                            strWhere += ") AND (";
                        }
                        else
                ...
                strWhere += ")) " + "\r\n";
                strKeyWordStartKbn = Def.DefTYPE_FIRST;
            }

            // 部署・設計
            // 設計部署
            DataRow[] drBusyo;
            SetSelectTable(Table, out drBusyo, Def.DefTYPE_BUSYO);
            ...
            if (drBusyo.Length  > 0)
            {
                ...
                else if (paraCategoryCondition == Def.DefTYPE_OR && Type != null
                    && strKeyWordStartKbn == Def.DefTYPE_FIRST) // 過去トラ検索結果　カテゴリの一番目
                {
                    strWhere += "AND ((";
                    strKeyWordStartKbn = Def.DefTYPE_NEXT;
                }
                else if (paraCategoryCondition == Def.DefTYPE_OR && Type == null) // 過去トラ検索
                {
                    strWhere += "OR (";
                }
                // 20170719 Add End
            ...
            // 評価部署
            DataRow[] drHyouka;
            SetSelectTable(Table, out drHyouka, Def.DefTYPE_HYOUKA);
            ...
            if (drHyouka.Length > 0)
            {
                ...
                if (!andFlg)
                {
                        ...
                        && strKeyWordStartKbn == Def.DefTYPE_FIRST) // 過去トラ検索結果　カテゴリの一番目
                    {
                        ...
                        && strKeyWordStartKbn == Def.DefTYPE_NEXT) // 過去トラ検索結果　カテゴリの一番目ではない
                    {
                        strWhere += "OR (";
                    }
                    else if (paraCategoryCondition == Def.DefTYPE_OR && Type == null) // 過去トラ検索
            ...
            // 部品・部位
            DataRow[] drParts;
            SetSelectTable(Table, out drParts, Def.DefTYPE_PARTS);
            ...
            // 開発符号
            DataRow[] drKaihatu;
            SetSelectTable(Table, out drKaihatu, Def.DefTYPE_KAIHATU);
            ...
            // 現象（分類）
            DataRow[] drGensyo;
            SetSelectTable(Table, out drGensyo, Def.DefTYPE_GENSYO);
            ...
            // 現象（制御系）
            DataRow[] drSGensyo;
            SetSelectTable(Table, out drSGensyo, Def.DefTYPE_SGENSYO);
            ...
            // 原因（分類）
            DataRow[] drGenin;
            SetSelectTable(Table, out drGenin, Def.DefTYPE_GENIN);
            ...
            // 車型特殊
            DataRow[] drSyakata;
            SetSelectTable(Table, out drSyakata, Def.DefTYPE_SYAKATA);
            ...
            // 要因（制御系）
            DataRow[] drSYouin;
            SetSelectTable(Table, out drSYouin, Def.DefTYPE_SYOUIN);
            ...
            // EGTM形式
            DataRow[] drEgtm;
            SetSelectTable(Table, out drEgtm, Def.DefTYPE_EGTM);
            ...
            if (Type != null && strKeyWordStartKbn == Def.DefTYPE_NEXT) // 過去トラ検索結果　カテゴリの最後尾
            {
                strWhere += ") " + "\r\n";
            }
            ...
            if (paraCategoryCondition == Def.DefTYPE_AND) //カテゴリ検索AND条件
            {
                strSql += strWhere.Substring(3); //"AND"以降の文字列取得
            }
            else if (paraCategoryCondition == Def.DefTYPE_OR) //カテゴリ検索OR条件
            {
